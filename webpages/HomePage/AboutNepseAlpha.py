...
class AboutNepseAphaLocator:
    address = (By.XPATH, "//p[contains(normalize-space(),'Krishna Galli') and contains(normalize-space(),'Pulchowk') and contains(normalize-space(),'Lalitpur 44700')]")
    mobile_num = (By.XPATH, "//a[contains(normalize-space(),'+977-9813667733')]")
    landline_num = (By.XPATH, "//a[text()='01-5919211']")
    email_address = (By.XPATH, "//a[contains(normalize-space(),'info@nepsealpha.com')]")
...
class AboutNepseAphaSection(BasePage):
    ...
    def is_address_visible_and_correct(self):
        ...
        if self.is_text_visible(AboutNepseAphaLocator.address, expected_address):
            print(f"The address matches the:\n{expected_address}")
        else:
